...
export function ResultsEntryForm({ selectedTests, montageData, onResultsSaved, onBack }: ResultsEntryFormProps) {
  ...
  const initializeResults = () => {
    ...
    selectedTests.forEach((test) => {
      ...
      montageData.nombresLecturas.forEach((lectura) => {
        newResults[test.id][lectura] = Array(montageData.numeroRepeticiones).fill(0)
      })
    })
    return newResults
  }
  ...
  const handleResultChange = (testId: string, lecturaName: string, repeticionIndex: number, value: number) => {
    const currentResults = getResults()
    const newResults = { ...currentResults }
    if (!newResults[testId]) newResults[testId] = {}
    if (!newResults[testId][lecturaName])
      newResults[testId][lecturaName] = Array(montageData.numeroRepeticiones).fill(0)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Registro de Resultados - {montageData.nombreMontaje}</CardTitle>
          <CardDescription>
            Montaje: {montageData.numeroMontaje} | OT: {selectedTests[0]?.ot} | Objetivo: {selectedTests[0]?.objetivo}
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Seleccione la lectura para ingresar los resultados</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={currentLectura.toString()} onValueChange={(value) => setCurrentLectura(Number.parseInt(value))}>
            ...
            {montageData.nombresLecturas.map((lectura, lecturaIndex) => (
              <TabsContent key={lecturaIndex} value={lecturaIndex.toString()} className="space-y-6">
                {isCurrentLecturaSaved && (
                  <Alert>
                    <Eye className="h-4 w-4" />
                    <AlertDescription>
                      Esta lectura ya ha sido guardada. Puede modificar los valores y guardar nuevamente.
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <div className="space-y-4">
                  <h4 className="text-lg font-medium">Resultados - {lectura}</h4>

                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse border border-gray-300">
                      <thead>
                        <tr className="bg-gray-50">
                          <th className="border border-gray-300 p-2 text-left">Prueba</th>
                          <th className="border border-gray-300 p-2 text-left">Producto</th>
                          {Array.from({ length: montageData.numeroRepeticiones }, (_, i) => (
                      ...
                      <tbody>
                        {selectedTests.map((test) => {
                          const testResults =
                            currentResults[test.id]?.[lectura] || Array(montageData.numeroRepeticiones).fill(0)
                          const promedio = testResults.reduce((sum, val) => sum + val, 0) / testResults.length

                          return (
                            <tr key={test.id}>
                              <td className="border border-gray-300 p-2">{test.prueba}</td>
                              <td className="border border-gray-300 p-2">{test.producto}</td>
                              {testResults.map((value, repIndex) => (
                                <td key={repIndex} className="border border-gray-300 p-1">
                                  <Input
                                    type="number"
                                    value={value}
                                    onChange={(e) =>
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
                    <div className="text-center">
                      ...
                      <div className="mt-4">
                        <Label htmlFor={`photo-${lecturaIndex}`} className="cursor-pointer">
                          <span className="mt-2 block text-sm font-medium text-gray-900">
                            {photos[lecturaIndex] ? photos[lecturaIndex]?.name : "Subir foto de la lectura"}
                          </span>
                        </Label>
                        <Input
                ...
                <div className="flex justify-end">
                  <Button onClick={handleSaveLectura} className="flex items-center gap-2">
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={onResultsSaved} className="flex items-center gap-2 bg-transparent">
            <Calculator className="h-4 w-4" />
            Ir a Cálculo de Eficacia
          </Button>
          <Button onClick={onResultsSaved} disabled={savedLecturas.size === 0} className="flex items-center gap-2">
            <Save className="h-4 w-4" />
            Guardar y Continuar
          </Button>
        </div>
