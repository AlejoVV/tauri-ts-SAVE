...
// Datos simulados
const mockLecturas = ["Lectura 1", "Lectura 2", "Lectura 3", "Lectura 4", "Lectura 5"]
...
export function ResultsEntryModal({ open, onOpenChange, montage, onResultsSaved }: ResultsEntryModalProps) {
  ...
  const farmInfo = mockFarmInfo[montage.ot as keyof typeof mockFarmInfo] || { finca: "N/A", especie: "N/A" }

  const handleResultChange = (pruebaId: string, repeticionIndex: number, value: number) => {
  ...
  const getTestigoResults = () => {
    return testigoResults.length > 0 ? testigoResults : Array(montage.numeroRepeticiones).fill(0)
  }
      ...
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto p-0">
        <div className="bg-white rounded-lg border-2 border-gray-200 p-8 space-y-8 shadow-lg">
          ...
          <div className="space-y-1 pb-4 border-b border-gray-200">
            <h1 className="text-2xl font-bold text-gray-800">Registro de Resultados - {montage.nombreMontaje}</h1>
            <p className="text-sm text-gray-600">
              <span className="font-semibold">Montaje:</span> {montage.numeroMontaje} |{" "}
              <span className="font-semibold">OT:</span> {montage.ot} | <span className="font-semibold">Objetivo:</span>{" "}
              {montage.objetivo}
            </p>
            <p className="text-sm text-gray-600">
              <span className="font-semibold">Especie vegetal:</span> {farmInfo.especie} |{" "}
              <span className="font-semibold">Finca:</span> {farmInfo.finca}
            </p>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-300 shadow-sm">
            <div className="flex gap-4">
              ...
              <div className="flex flex-col gap-3 w-[120px]">
                ...
                <div className="h-[96px]"></div> {/* Adjusted height for alignment */}
                {/* Réplicas */}
                {Array.from({ length: montage.numeroRepeticiones }, (_, index) => (
                  <div
                    key={index}
              ...
              <div className="flex flex-col gap-3">
                ...
                {Array.from({ length: montage.numeroRepeticiones }, (_, repIndex) => {
                  ...
                  return (
                    <Input
                      ...
                      onChange={(e) => handleTestigoChange(repIndex, Number.parseFloat(e.target.value) || 0)}
                ...
                <div className="bg-emerald-100 border border-emerald-300 rounded-lg p-3 text-center h-12 flex items-center justify-center shadow-sm w-24">
                  <div className="text-lg font-bold text-emerald-800">{calculateTestigoAverage()}</div>
                </div>
              </div>

              {/* Columnas de productos */}
              {montage.pruebas.slice(0, 3).map((prueba, pruebaIndex) => {
                const productInfo = mockProductInfo[prueba as keyof typeof mockProductInfo] || {
                  dosis: "N/A",
                  unidades: "N/A",
                ...
                return (
                  <div key={prueba} className="flex flex-col gap-3">
                    ...
                    <div className="bg-gray-200 border border-gray-300 rounded-lg p-3 text-center h-[96px] flex flex-col justify-center shadow-sm w-24">
                      <div className="font-bold text-base text-gray-900">{prueba}</div>
                      <div className="text-xs text-gray-600 font-medium">{productInfo.nombre}</div>{" "}
                      {/* Changed text-sm to text-xs */}
                      <div className="text-sm font-semibold text-gray-800">{productInfo.dosis}</div>
                      <div className="text-[0.6rem] text-gray-600 leading-none">{productInfo.unidades}</div>{" "}
                    ...
                    {Array.from({ length: montage.numeroRepeticiones }, (_, repIndex) => (
                      <Input
                        ...
                        onChange={(e) => handleResultChange(prueba, repIndex, Number.parseFloat(e.target.value) || 0)}
                    ...
                    {/* Campo de promedio */}
                    <div className="bg-emerald-100 border border-emerald-300 rounded-lg p-3 text-center h-12 flex items-center justify-center shadow-sm w-24">
                      <div className="text-lg font-bold text-emerald-800">{calculateAverage(pruebaResults)}</div>
                    </div>
                  </div>
                )
              })}
          ...
          <div className="grid grid-cols-4 gap-6">
              ...
              <div className="text-center space-y-4">
                <div className="border-2 border-dashed border-gray-400 rounded-lg p-8 hover:border-gray-500 transition-colors cursor-pointer">
                  <Label htmlFor="photo-upload" className="cursor-pointer">
                    ...
                    <div className="text-base font-semibold text-gray-700">Subir fotos de la</div>
                    <div className="text-base font-semibold text-gray-700">lectura</div>
                  </Label>
                  <Input
                    id="photo-upload"
                    type="file"
            ...
            <div className="col-span-2 grid grid-cols-2 gap-4">
              {photos.slice(0, 2).map((photo, index) => (
                <div key={index} className="bg-gray-200 rounded-xl overflow-hidden border border-gray-300 shadow-sm">
