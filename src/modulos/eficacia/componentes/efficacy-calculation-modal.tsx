...
const mockLecturas = ["Lectura Inicial", "Lectura 7 días", "Lectura 14 días", "Lectura Final"]
const mockResults = {
  "1206": {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Cálculo de Eficacia - {montage.nombreMontaje}</DialogTitle>
          <DialogDescription>
            Montaje: {montage.numeroMontaje} | Seleccione las lecturas y método para calcular la eficacia
          </DialogDescription>
        </DialogHeader>
                ...
                <table className="w-full border-collapse border border-gray-300">
                  ...
                  <tbody>
                    {efficacyResults.map((result: any, index: number) => (
                      <tr key={index}>
                        <td className="border border-gray-300 p-3">{result.prueba}</td>
                        <td className="border border-gray-300 p-3">{result.producto}</td>
