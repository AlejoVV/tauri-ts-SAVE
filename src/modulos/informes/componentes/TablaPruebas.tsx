...
function EstadoPrueba({ estado }: { estado: string }) {
  const getEstadoColor = (estado: string) => {
    switch (estado.toLowerCase()) {
      case "en_curso": return "bg-blue-100 text-blue-800";
      case "completada": return "bg-green-100 text-green-800";
      case "esperando aprobación": return "bg-yellow-100 text-yellow-800";
      case "cancelada": return "bg-red-100 text-red-800";
      default: return "bg-gray-100 text-gray-800";
    }
  };
  return (
    <span className={cn("inline-flex px-2 py-1 text-xs font-medium rounded", getEstadoColor(estado))}>
      {estado}
    </span>
  );
}

export function TablaPruebas({ pruebas, onSelectionChange }: TablaPruebasProps) {
    ...
    () => [
      {
        accessorKey: "finca",
        header: "Finca",
        minSize: 80, maxSize: 300, size: 100,
        enableColumnFilter: true, filterFn: "contains",
        Cell: ({ cell }) => (
          <div className={cn("text-sm truncate")} title={cell.getValue<string>()}>
      ...
      },
      {
        ...
        header: "Objetivo",
        minSize: 120, maxSize: 200, size: 150,
        enableColumnFilter: true, filterFn: "contains",
        Cell: ({ cell }) => (
          <div className={cn("text-sm truncate")} title={cell.getValue<string>()}>
            {cell.getValue<string>()}
      ...
      },
      {
        ...
        minSize: 150, maxSize: 300, size: 200,
        enableColumnFilter: true, filterFn: "contains",
        Cell: ({ cell }) => (
          <div className={cn("text-sm truncate")} title={cell.getValue<string>()}>
            {cell.getValue<string>()}
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: () => (
      <div className={cn("flex items-center gap-4")}>
        ...
        {filasSeleccionadas > 0 && (
          <div className={cn("flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1")}>
            <span className={cn("text-sm font-medium text-blue-700")}>
    ...
    muiTableContainerProps: { sx: { maxHeight: "calc(100% - 60px)", height: "auto", width: "100%", overflow: "auto" } },
    muiTableProps: { sx: { tableLayout: "fixed", width: "100%" } },
    muiTopToolbarProps: { sx: { minHeight: "56px", maxHeight: "56px", flexShrink: 0 } },
    muiTableHeadProps: { sx: { position: "sticky", top: 0, zIndex: 10, backgroundColor: "#f8fafc", borderBottom: "2px solid #e2e8f0" } },
    muiTableHeadCellProps: { sx: { fontWeight: 600, fontSize: "0.875rem", color: "#374151", padding: "8px 12px", backgroundColor: "#f8fafc", position: "sticky", top: 0, zIndex: 10, borderBottom: "2px solid #e2e8f0" } },
    muiTableBodyProps: { sx: { "& .MuiTableRow-root": { height: "10px", minHeight: "10px", "&:hover": { backgroundColor: "#f1f5f9" } } } },
    muiTableBodyCellProps: { sx: { padding: "1px 4px", fontSize: "0.8rem", height: "10px", minHeight: "10px", borderBottom: "1px solid #e5e7eb", lineHeight: "1.2" } },
    muiFilterTextFieldProps: { size: "small", variant: "outlined" },
  ...
  if (pruebas.length === 0) {
    return (
      <div className={cn("h-full flex items-center justify-center")}>
        <div className={cn("bg-white border border-gray-200 rounded-lg p-8 text-center")}>
          <p className={cn("text-gray-500")}>No se encontraron pruebas en curso para esta OT.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("w-full h-full flex flex-col overflow-hidden rounded-lg border")}>
