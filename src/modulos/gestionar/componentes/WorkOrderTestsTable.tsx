...
import { obtenerPruebasPorOrden, type VistaMaestraRow } from "@/modulos/gestionar/servicios";
import { formatDate } from "@/modulos/gestionar/utils";
import { TypedEstadoBadge } from "@/modulos/gestionar/componentes/EstadoBadge";
...
export function WorkOrderTestsTable({
  ...
  // `ordenTrabajo` es un número primitivo; evita depender del objeto `otData` que sería una referencia inestable.
  useEffect(() => {
    if (!ordenTrabajo) {
      setData([]);
      setRowSelection({});
  ...
  const columns = useMemo<MRT_ColumnDef<VistaMaestraRow>[]>(
    () => [
      ...
      },
      {
        ...
        minSize: 30,
        Cell: ({ cell }) => (
          <TypedEstadoBadge estado={cell.getValue<string>()} type="lab" />
        ),
      },
      {
      ...
      },
      {
        ...
        minSize: 30,
        Cell: ({ cell }) => (
          <TypedEstadoBadge estado={cell.getValue<string>()} type="fact" />
        ),
      },
      {
        ...
        minSize: 30,
        Cell: ({ cell }) => (
          <TypedEstadoBadge estado={cell.getValue<string>()} type="ot" />
        ),
      },
      {
        ...
        minSize: 30,
        Cell: ({ cell }) => (
          <TypedEstadoBadge estado={cell.getValue<string>()} type="proceso" />
        ),
      },
      {
